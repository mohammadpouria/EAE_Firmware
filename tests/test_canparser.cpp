...
TEST(CANParserTest, ParsesPositiveTemperature) {
    ...
    frame.dlc = 2;
    frame.payload = {0x21, 0x66};
    
    EXPECT_TRUE(parser.parseFrame(frame));
    EXPECT_DOUBLE_EQ(parser.read().temperature_c, 85.50);
}
...
TEST(CANParserTest, ParsesNegativeTemperature) {
    ...
    frame.dlc = 2;
    frame.payload = {0xFC, 0x18};
    
    parser.parseFrame(frame);
    EXPECT_DOUBLE_EQ(parser.read().temperature_c, -10.00);
}
...
TEST(CANParserTest, ParsesLevelFault) {
    ...
    frame.dlc = 1;
    frame.payload = {0x00}; // 0 = Fault, 1 = OK
    
    parser.parseFrame(frame);
    EXPECT_FALSE(parser.read().level_ok);
}